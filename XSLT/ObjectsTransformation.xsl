...
Production-->
            "produced_by": {
                ...
                "timespan": {
                    ...
                    "begin_of_the_begin": "<xsl:choose><xsl:when test="string-length(atom[@name='CreEarliestDate']) &#60;= 5 and string-length(atom[@name='CreEarliestDate']) &#62; 0"><xsl:value-of select="format-number(atom[@name='CreEarliestDate'], '0000')"/>-01-01T00:00:00</xsl:when><xsl:when test="string-length(atom[@name='CreEarliestDate']) &#62;= 6"><xsl:value-of select="atom[@name='CreEarliestDate']"/>T00:00:00</xsl:when></xsl:choose>"</xsl:if><xsl:if test="atom[@name='CreLatestDate'] != '' and not(ends-with(atom[@name='CreLatestDate'], '-'))">,
                    "end_of_the_end": "<xsl:choose><xsl:when test="string-length(atom[@name='CreLatestDate']) &#60;= 5 and string-length(atom[@name='CreLatestDate']) &#62; 0"><xsl:value-of select="format-number(atom[@name='CreLatestDate'], '0000')"/>-12-31T00:00:00</xsl:when><xsl:when test="string-length(atom[@name='CreLatestDate']) &#62;= 6"><xsl:value-of select="atom[@name='CreLatestDate']"/>T00:00:00</xsl:when></xsl:choose>"</xsl:if>
                }</xsl:if>
